...
#eval/TUM/0_10_ss5t_0.300000e_0.010000g_0.100000
#path = "../results/"
path = os.getcwd()
os.chdir(path)

referenceply = PlyData.read(path + "/Template-Update_3.ply")
templateply = PlyData.read(path +  "/Template-Update_4.ply")
frames = 250
...
reference.set_data(ref, edge_color=None, face_color="red" , size=5)
view.add(reference)
...
template.set_data(tem, edge_color=None, face_color="blue", size=5)
view.add(template)
...
def update(ev):
	...
	if it < 10:
		...
		s = '0' + s
	templateply = PlyData.read(path + "/Template-Update_" + str(it) +  ".ply")
	tem = np.column_stack((templateply['vertex']['x'],templateply['vertex']['y']))
	tem = np.column_stack((tem, templateply['vertex']['z']))   	
	template.set_data(tem, edge_color=None, face_color="blue", size=5)
	view.add(template)
...
#text = visuals.Text("HELLO", parent=canvas.scene , color='green', pos = [canvas.size[0] // 2, canvas.size[1] // 3], font_size=100)
#view.add(text)
