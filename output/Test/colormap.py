...
def shiftedColorMap(cmap, start=0, midpoint=0.05, stop=1.0, name='shiftedcmap'):
    ...
    shift_index = np.hstack([
        np.linspace(0.0, 1-midpoint, 128, endpoint=False), 
        np.linspace(1-midpoint, 1.0, 129, endpoint=True)
    ])
...
for i in range(2, 72):
    ...
    shiftedJet_cmap = shiftedColorMap(orig_Jetcmap, start=0.0, midpoint=0.5, stop=1.0, name='shifted')
